...
def train_model(input_model, model_save_path, args, *loaders):
    ...
    early_stopping = EarlyStopping(patience=args.patience, verbose=False, path=model_save_path)
    ...
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[int(args.epochs*0.4), int(args.epochs*0.6), int(args.epochs*0.8)], gamma=0.4)
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[5, 10], gamma=0.5)
    ...
    ---------------------------------------------------------------------------""")
        ...
        if ((epoch+1) >= 10) and ((epoch+1) % 5 == 0):
            save_path = os.path.join(model_save_path, f'model_ckpt_{epoch+1}.pth')
